...
* combine racer strength and external conditions into calibrated first-place and second-place probabilities;
* compare pre-race probabilities with market odds separately;
* update ratings only after race results become available.

Prediction accuracy, reproducibility, and prevention of time leakage take priority over architectural complexity.

## Core principles

### 1. Time leakage is a blocking error

Every historical prediction must reproduce what was actually knowable at the prediction timestamp.
...
* exhibition and pre-race timing information must use only information published before the prediction;
...
If temporal availability is uncertain, treat the feature as unavailable until verified.
...
Store enough information to reproduce the exact rating state used for every historical prediction.

Do not choose a rating formula by intuition alone.

Compare candidate rating methods using out-of-sample predictive performance.
...
Do not assume that stronger wind or higher waves have the same effect at every venue or every course.

Prefer statistically supported interactions and regularization over large collections of manually invented rules.
...
Do not assume that a timing variable is useful merely because its raw correlation is strong.
...
* first-place probabilities must form a coherent probability distribution;
* second-place probabilities must form a coherent probability distribution.
...
Then compare those probabilities with the odds snapshot available at the decision timestamp.

If a future market-aware model is investigated, treat it as a separate model and benchmark it independently.
...
Backtests must use only odds that would actually have been available at the simulated decision time.
...
Missing values are missing values unless a documented rule explicitly defines an imputation.
...
Do not restore the old Boatrace architecture merely because old artifacts exist.

Legacy dumps, CSV files, and definition documents may be reused as data or reference material after their contents are verified.

Do not depend on unidentified legacy schemas, materialized views, security artifacts, orchestration infrastructure, or historical code without explicit justification.
...
Do not manually calculate large statistical results in the agent context.
...
A more complex model is accepted only when it provides reproducible out-of-sample improvement.
...
Do not add frameworks, services, abstractions, databases, agents, or dependencies unless they solve a current requirement.

Reuse stable Python, PostgreSQL, and standard statistical tooling before introducing custom infrastructure.

When showing Python execution commands on Windows, use the full path to the project Python executable when it is known.
...
test probability bounds, normalization, calibration inputs, and determinism where applicable.

Do not create large audit/evidence systems solely to prove that routine work was performed.

## Credit efficiency

Default to GPT-6 Sol at xhigh reasoning effort.

When subagents are useful and no stronger role-specific model is required, default to GPT-6 Luna at max reasoning effort.
...
Use subagents only when independent parallel work materially reduces time or improves correctness.

Do not spawn agents merely to repeat work already performed by the primary agent.

Use reasoning beyond the normal Sol xhigh baseline only when the task genuinely requires it, such as:
...
Return to the normal Sol xhigh baseline after any exceptional escalation is resolved.
...
If evidence is insufficient, state that it is insufficient rather than inventing certainty.
